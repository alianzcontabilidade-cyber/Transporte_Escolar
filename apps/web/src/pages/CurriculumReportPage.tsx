...
import { loadMunicipalityData, loadSchoolData, printReportHTML, openReportAsPDF, generateReportHTML } from '../lib/reportTemplate';
...
export default function CurriculumReportPage() {
  ...
  const handlePDF = async () => { const html = buildReportHTML(); if (html) await openReportAsPDF(html, 'Quadro_Curricular'); };
  ...
  return (
    <div className="p-6">
      ...
      <ExportModal open={!!pgExportModal} onClose={() => setPgExportModal(null)} onExport={(fmt: any) => { if (pgExportModal?.html) { handleExport(fmt, [], pgExportModal.html, pgExportModal.filename); } setPgExportModal(null); }} title="Exportar Quadro Curricular" />
    </div>
  );
}
