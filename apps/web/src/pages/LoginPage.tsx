...
export default function LoginPage() {
  ...
  const [screen, setScreen] = useState<'login' | 'recover' | 'sent'>('login');
  ...
  const handleLogin = async () => {
    ...
    catch (e: any) { setError(e?.message || 'Credenciais invalidas.'); }
    setLoading(false);
  };

  const handleRecover = async () => {
    if (!recoverEmail) { setError('Informe o e-mail.'); return; }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          {screen === 'login' && (<>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <button type="button" onClick={() => setShowDrop(v => !v)} className="input flex items-center justify-between w-full text-left">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 overflow-hidden">
                    {ID_TYPES.map(t => (
                      <button key={t.value} type="button"
                        onClick={() => { setIdType(t.value); setIdValue(''); setShowDrop(false); }}
                        className={'w-full flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-primary-50 ' + (idType === t.value ? 'bg-primary-50 text-primary-600 font-medium' : 'text-gray-700')}>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                <input className="input pl-9" placeholder={cur.placeholder} value={idValue}
            ...
            <div className="mb-2">
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} className="input pr-10" placeholder="Sua senha"
          ...
          {screen === 'recover' && (<>
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">Informe o e-mail e enviaremos as instrucoes.</p>
            </div>
            {error && <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 text-sm rounded-xl">{error}</div>}
            <div className="mb-6">
              ...
              <div className="relative">
                ...
                <input className="input pl-9" type="email" placeholder="seu@email.com"
            ...
            <button onClick={handleRecover} disabled={recoverLoading}
              className="btn-primary w-full py-3 text-base font-semibold disabled:opacity-50">
              {recoverLoading ? 'Enviando...' : 'Enviar instrucoes'}
            </button>
          </>)}
          {screen === 'sent' && (
            <div className="text-center py-4">
              ...
              <p className="text-gray-500 text-sm mb-2">Enviamos as instrucoes para:</p>
              <p className="font-semibold text-primary-600 mb-6">{recoverEmail}</p>
              <p className="text-xs text-gray-400 mb-6">Verifique tambem o spam. Link expira em 30 min.</p>
              <button type="button"
                onClick={() => { setScreen('login'); setRecoverEmail(''); setError(''); }}
                className="btn-primary w-full py-3">Voltar ao login</button>
            </div>
          )}
        </div>
        <p className="text-center text-xs text-gray-400 mt-6">
          TransEscolar &copy; {new Date().getFullYear()} &middot; Sistema Municipal de Transporte Escolar
        </p>
      </div>
    </div>
  );
