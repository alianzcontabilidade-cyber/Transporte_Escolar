...
export default function LowPerformancePage() {
  ...
  const handlePDF = async () => { const html = buildReportHTML(); if (html) await openReportAsPDF(html, 'Alunos_Baixo_Rendimento'); };
  ...
  return (
    <div className="p-6">
      ...
      <ExportModal open={!!pgExportModal} onClose={() => setPgExportModal(null)} onExport={(fmt: any) => { if (pgExportModal?.html) { handleExport(fmt, [], pgExportModal.html, pgExportModal.filename); } setPgExportModal(null); }} title="Exportar Relatório de Baixo Rendimento" />
    </div>
  );
}
