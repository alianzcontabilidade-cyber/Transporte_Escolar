  ...
  return (
    <div className="p-6">
      ...
      <div className="card p-4 mb-5">
        <div className="flex items-center justify-between mb-3"><h2 className="font-semibold text-gray-800 flex items-center gap-2"><Navigation size={16} className="text-primary-500"/> Mapa ao Vivo</h2><span className="text-xs text-gray-400">{busLocations.size} ônibus com GPS ativo</span></div>
          ...
          {!trips.length ? (
            ...
            <div className="space-y-3">
              {trips.map((item: any) => {
                ...
                const isSel = selectedTrip?.trip.id===item.trip.id;
                return (
                  <div key={item.trip.id} onClick={() => setSelectedTrip(isSel?null:item)} className={`card cursor-pointer transition-all ${isSel?'border-primary-300 bg-primary-50/30':'hover:border-gray-300'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <ChevronRight size={14} className={isSel?'text-primary-500 rotate-90':'text-gray-300'}/>
                      </div>
                    </div>
                    {isSel&&<div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-3 gap-2"><div className="text-center p-2 bg-gray-50 rounded-lg"><p className="text-xs text-gray-500">Alunos</p><p className="font-bold">{item.students?.length||0}</p></div><div className="text-center p-2 bg-gray-50 rounded-lg"><p className="text-xs text-gray-500">Paradas</p><p className="font-bold">{item.stops?.length||0}</p></div><div className="text-center p-2 bg-gray-50 rounded-lg"><p className="text-xs text-gray-500">Status</p><p className="font-bold text-xs">{sl(item.trip.status)}</p></div></div>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="card p-0 overflow-hidden">
            {!events.length?(<div className="p-6 text-center text-gray-400 text-sm"><Activity size={28} className="mx-auto mb-2 text-gray-200"/><p>Sem eventos recentes</p></div>):(
              <div className="divide-y divide-gray-50 max-h-80 overflow-y-auto">
                {events.map((ev,i) => (<div key={i} className="flex items-start gap-3 px-4 py-3"><div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${ev.type==='board'?'bg-green-100':'bg-blue-100'}`}>{ev.type==='board'?<CheckCircle size={14} className="text-green-600"/>:<MapPin size={14} className="text-blue-600"/>}</div><div className="flex-1 min-w-0"><p className="text-xs font-medium text-gray-700">{ev.type==='board'?'Aluno embarcou':'Parada chegou'}</p><p className="text-xs text-gray-500 truncate">{ev.studentName||ev.stopName||'—'}</p><p className="text-xs text-gray-400">{new Date(ev.time).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}</p></div></div>))}
              </div>
            )}
