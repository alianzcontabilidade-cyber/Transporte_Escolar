...
export default function SubjectPerformancePage() {
  ...
  return (
    <div className="p-6">
      ...
      <ExportModal allowSign={true} open={!!pgExportModal} onClose={() => setPgExportModal(null)} onExport={(fmt: any) => { if (pgExportModal?.html) { handleExport(fmt, [], pgExportModal.html, pgExportModal.filename); } setPgExportModal(null); }} title="Exportar Desempenho por Disciplina" />
    </div>
  );
}
