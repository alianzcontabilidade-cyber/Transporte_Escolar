...
app.get('/api/seed-fix-grades', async (_req, res) => {
    try {
        ...
        const [cls] = await index_1.db.execute(drizzle_orm_1.sql.raw(`SELECT c.id, c.schoolId, c.name as turma, c.classShift as shift, cg.name as grade FROM classes c JOIN class_grades cg ON c.classGradeId=cg.id WHERE c.municipalityId=1`));
        const [subj] = await index_1.db.execute(drizzle_orm_1.sql.raw(`SELECT id FROM subjects WHERE municipalityId=1 LIMIT 8`));
        ...
        const [enr] = await index_1.db.execute(drizzle_orm_1.sql.raw(`SELECT studentId, classId FROM enrollments WHERE municipalityId=1 AND enrollmentStatus='active'`));
        log.push(enr?.length + ' matriculas');
        // Notas em batch
        const gradeValues = [];
        for (const e of (enr || [])) {
            const classAss = (allAss || []).filter((a) => a.classId === e.classId);
