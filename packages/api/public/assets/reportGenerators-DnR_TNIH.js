import{g as S}from"./reportTemplate-yQJhTdV8.js";const P=["janeiro","fevereiro","março","abril","maio","junho","julho","agosto","setembro","outubro","novembro","dezembro"];function T(e){if(!e)return"--";const i=typeof e=="string"?new Date(e):e;return isNaN(i.getTime())?String(e):`${i.getDate().toString().padStart(2,"0")}/${(i.getMonth()+1).toString().padStart(2,"0")}/${i.getFullYear()}`}function M(e){if(!e)return"--";const i=typeof e=="string"?new Date(e):e;return isNaN(i.getTime())?String(e):`${i.getDate()} de ${P[i.getMonth()]} de ${i.getFullYear()}`}function a(e,i){return`<div class="field-row"><span class="field-label">${e}</span><span class="field-value">${i||"--"}</span></div>`}function A(e){return e==="morning"?"MATUTINO":e==="afternoon"?"VESPERTINO":e==="evening"?"NOTURNO":e==="full_time"?"INTEGRAL":e||"--"}function L(e){if(!e)return"";const i=e.toLowerCase();return i.includes("berçário")||i.includes("bercario")||i.includes("maternal")||i.includes("jardim")||i.includes("pré")||i.includes("pre")||i.includes("creche")||i.includes("infantil")?"EDUCAÇÃO INFANTIL":i.includes("6º")||i.includes("7º")||i.includes("8º")||i.includes("9º")||i.includes("6 ano")||i.includes("7 ano")||i.includes("8 ano")||i.includes("9 ano")?"ENSINO FUNDAMENTAL – 6º ao 9º Ano":i.includes("1º")||i.includes("2º")||i.includes("3º")||i.includes("4º")||i.includes("5º")||i.includes("1 ano")||i.includes("2 ano")||i.includes("3 ano")||i.includes("4 ano")||i.includes("5 ano")?"ENSINO FUNDAMENTAL – 1º ao 5º Ano":i.includes("médio")||i.includes("medio")||i.includes("1ª série")||i.includes("2ª série")||i.includes("3ª série")?"ENSINO MÉDIO":i.includes("eja")||i.includes("supletivo")?"EDUCAÇÃO DE JOVENS E ADULTOS (EJA)":"ENSINO FUNDAMENTAL"}function w(e,i,l,s,n){const p=L(e.grade),m=new Date().getFullYear(),d=e.sex==="M"?"o":e.sex==="F"?"a":"o(a)",$=e.sex==="M"?"filho":e.sex==="F"?"filha":"filho(a)",v=e.sex==="M"?"nascido":e.sex==="F"?"nascida":"nascido(a)",f=e.naturalness?`natural de <b>${e.naturalness}${e.naturalnessUf?" – "+e.naturalnessUf:""}</b>, `:"",r=`
  ...
  `;return S({municipality:l,secretaria:s,school:i,title:"DECLARAÇÃO DE ESCOLARIDADE",content:r,signatories:n,fontFamily:"serif",fontSize:13})}function B(e,i,l,s,n,p){const m=`
    <p class="declaration-text">
  ...
  `;return S({municipality:l,secretaria:s,school:i,title:"DECLARAÇÃO DE TRANSFERÊNCIA",content:m,signatories:n,fontFamily:"serif",fontSize:13})}function _(e,i,l,s,n,p){const m=`
    <p class="declaration-text">
      Declaramos, para os devidos fins e efeitos legais, que <b>${e.name||"ALUNO(A)"}</b>,
  ...
  `;return S({municipality:l,secretaria:s,school:i,title:"DECLARAÇÃO DE FREQUÊNCIA",content:m,signatories:n,fontFamily:"serif",fontSize:13})}function q(e,i,l,s,n,p,m){const d=m||new Date().getFullYear();let $=`<div class="student-info">
    ...
    <div class="si-detail">Ano Letivo: ${d}${e.birthDate?" | Data de Nascimento: "+T(e.birthDate):""}</div>
  </div>`,v="",f=0,r=0;for(const o of i){const g=o.b1??o.bim1??"--",y=o.b2??o.bim2??"--",t=o.b3??o.bim3??"--",h=o.b4??o.bim4??"--",N=[g,y,t,h].map(b=>typeof b=="number"?b:parseFloat(b)).filter(b=>!isNaN(b)),c=N.length>0?N.reduce((b,F)=>b+F,0)/N.length:null,I=o.faltas??o.absences??"--",E=c!==null?c>=6?"Aprovado":c>=0?"Reprovado":"--":"--",R=E==="Aprovado"?"approved":E==="Reprovado"?"failed":"";c!==null&&(f+=c,r++),v+=`<tr>
      ...
      <td>${I}</td>
      <td class="${R}">${E}</td>
    </tr>`}const x=r>0?(f/r).toFixed(1):"--",D=r>0&&f/r>=6?"APROVADO(A)":r>0?"REPROVADO(A)":"--",C=`
  ...
  `;return S({municipality:s,secretaria:n,school:l,title:"BOLETIM ESCOLAR",subtitle:`Ano Letivo ${d}`,content:$+C,signatories:p,fontFamily:"sans-serif",fontSize:12})}function H(e,i,l,s,n,p){var D,C;const m=L(e.grade),d=`
  ...
  `;let $="";for(const o of i){const g=(D=o.result)!=null&&D.toLowerCase().includes("aprov")?"approved":(C=o.result)!=null&&C.toLowerCase().includes("reprov")?"failed":"";$+=`<tr>
      <td>${o.year}</td>
  ...
  `;let f="";const r=i.filter(o=>o.grades&&o.grades.length>0);if(r.length>0)for(const o of r){let g="",y=0;for(const t of o.grades||[]){const h=t.b1??t.bim1??"--",O=t.b2??t.bim2??"--",N=t.b3??t.bim3??"--",c=t.b4??t.bim4??"--",E=[h,O,N,c].map(u=>typeof u=="number"?u:parseFloat(u)).filter(u=>!isNaN(u)),R=E.length>0?E.reduce((u,U)=>u+U,0)/E.length:null,b=t.cargaHoraria||t.weeklyHours?(t.weeklyHours||0)*40:80;y+=b;const F=t.faltas??t.absences??"--";g+=`<tr>
  ...
  `;return S({municipality:s,secretaria:n,school:l,title:"HISTÓRICO ESCOLAR",subtitle:m||void 0,content:d+v+f+x,signatories:p,fontFamily:"sans-serif",fontSize:11})}function G(e,i,l,s,n){const p=`
  ...
  `;return S({municipality:l,secretaria:s,school:i,title:"FICHA DE MATRÍCULA",subtitle:e.enrollmentType==="novato"?"NOVATO (PRIMEIRA MATRÍCULA)":e.enrollmentType==="renovacao"?"RENOVAÇÃO DE MATRÍCULA":"MATRÍCULA POR TRANSFERÊNCIA",content:p,signatories:n,fontFamily:"sans-serif",fontSize:11})}function k(e,i,l,s,n,p){const m=`<div class="student-info">
  ...
  </div>`;let d="",$=0,v=0;e.sort((r,x)=>(r.name||"").localeCompare(x.name||"")),e.forEach((r,x)=>{r.sex==="M"?$++:r.sex==="F"&&v++,d+=`<tr>
      <td>${x+1}</td>
  ...
  `;return S({municipality:s,secretaria:n,school:l,title:"RELAÇÃO DE ALUNOS MATRICULADOS POR TURMA",subtitle:`${i.grade} - Turma ${i.className} - ${A(i.shift)} - ${i.year}`,content:m+f,signatories:p,fontFamily:"sans-serif",fontSize:11})}export{H as a,G as b,_ as c,B as d,w as e,k as f,q as g};
